...
CLASSIFIER_MODEL_NAME = env("CLASSIFIER_MODEL_NAME", "gpt-4.1-mini")
...
CLASSIFIER_MODEL = ChatOpenAI(model=CLASSIFIER_MODEL_NAME, temperature=0)


# Canonical columns mapping (user-provided variants -> canonical)
...
# Helpers (safe LLM text extraction, serialization)
...
INTENT_CLASSIFIER_PROMPT = r"""
You are an advanced intent classifier. 
Your job is to determine what the user truly wants based on semantics, meaning, and context — 
not just keywords.

You must classify the user’s question into EXACTLY one of the following intents:
...
1. sql_generation
   ...
   - Filtering by item_no or attributes
   - Any question requiring database values
   - Anything requiring SQL to access structured data
   - Summary of data stored in the database

2. general_answer
   When the user’s question is:
   - About technology, ML, coding, life, career, concepts, opinions
   - Definitions, explanations, guidance, how-to instructions
   - ChatGPT-style conversation that does NOT require database values

3. both
   When the question has two parts:
   - A general question + a database-dependent question
   Example:
     "What is diamond clarity and also show clarity distribution from database."
...
RULES:
- Think semantically, not by keywords alone.
- Consider phrasing, meaning, user intent and context.
- Output ONLY one of: sql_generation, general_answer, both, clarification_needed.
"""
...
You are a production-grade PostgreSQL query generator. Produce ONE valid SELECT statement only.
Placeholders: {schema}, {table}, {columns}, {memory}, {question}

Rules:
- Use ONLY columns from {columns} (exact names).
- Wrap column names in double quotes, e.g. "item_no".
- If question contains an item identifier (SKU-like) treat as item lookup and add WHERE "item_no" = '...'.
- If the question asks for totals/sum/average/min/max, use SUM/AVG/MIN/MAX on the mentioned column.
- If question asks for "all details", "full details", "overview", "table details", or similar, output SELECT * FROM {schema}.{table}
- Always include "item_no" in the SELECT when filtering by item_no.
- Do NOT add semicolons or any explanation.
Now generate the SELECT for:
Question: {question}
"""


# ANSWER PROMPT (updated, but used in generate_answer later)
...
2. Answer data questions using ONLY the content inside {rows}.

------------------------------------------
GENERAL QUESTION MODE:
- If the user’s question does NOT depend on the database,
  respond normally like a helpful assistant.
- DO NOT use {rows} for general questions.
- You may explain, suggest, and give examples.

------------------------------------------
DATA QUESTION MODE (activated when SQL has run):
- Use ONLY the data available in {rows}.
- Never invent data.
- Never mention SQL, queries, databases, or tables.
...
"summary", "all details", "full details", 
"database details", "table overview", 
"complete data", "full data"

In this mode:
- Give a clear summary of the dataset.
- Include number of rows.
- List all detected column names.
- For numeric fields, compute min/max/avg/sum ONLY if possible from {rows}.
...
- If one row: show its details.
- If multiple rows: summarize and list important values.
- Keep formatting simple (bullets or key-value pairs).

Always follow these rules strictly.
"""


# SQL sanitizers and utilities
...
# Extract requested fields using canonical mapping

def extract_requested_fields(question: str, available_cols: List[str]) -> List[str]:
    q = question.lower()
    found: List[str] = []
    # direct lookups using canonical names and simple tokens
    for col in available_cols:
        token = col.lower().replace(" ", "_")
        if token in q or col.lower() in q:
            found.append(col)
    # try mapping common phrases to canonical columns
    synonyms = {
        "inventory cost": "inventory_cost_final",
        "inventory_cost_final": "inventory_cost_final",
        "secondary sales total cost": "secondary_sales_total_cost",
    ...
    # include canonical lookup via CANONICAL_COLUMNS variants
    for phrase, canonical in CANONICAL_COLUMNS.items():
        if phrase in q and canonical in available_cols and canonical not in found:
            found.append(canonical)
    for syn, col in synonyms.items():
        if syn in q and col not in found and col in available_cols:
            found.append(col)
    # Keep order unique
    uniq = []
    for f in found:
...
async def generate_sql(session_id: str, question: str) -> str:
    try:
        ...
        return "SELECT 1"
    qlow = question.lower()
    # quick 'columns' or 'schema' introspection
    if "columns" in qlow or "database" in qlow or "schema" in qlow:
...
# Answer generation - uses ANSWER_MODEL
...
async def generate_answer(question: str, sql: str, rows: List[Dict[str, Any]]) -> str:
    ...
    try:
        ...
        if not rows:
            return "I’m sorry, but I couldn't find the information you requested.\n\nYou may also ask:\n• Provide details about a specific item_no.\n• Give me a summary of the database."
        return simple_summary_text(rows)


# Simple summary generator (fallback + always included)

def simple_summary_stats(rows: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for r in rows:
        for k, v in r.items():
            ...
            if isinstance(v, (int, float)):
                ...
                try:
                    f = float(v)
                    numeric.setdefault(k, []).append(f)
                except Exception:
                    categorical.setdefault(k, []).append(str(v))
    numeric_summary = {}
    for k, vals in numeric.items():
        ...
        zeros = sum(1 for x in vals if x == 0)
        numeric_summary[k] = {"count": cnt, "min": mn, "max": mx, "avg": avg, "zeros": zeros}
    categorical_summary = {}
    for k, vals in categorical.items():
        ...
        unique = len(c)
        categorical_summary[k] = {"unique": unique, "top": most[:5]}
    return {"total_rows": len(rows), "numeric": numeric_summary, "categorical": categorical_summary}

def simple_summary_text(rows: List[Dict[str, Any]]) -> str:
    s = simple_summary_stats(rows)
    if s["total_rows"] == 0:
        return "I’m sorry, but I couldn't find the information you requested."
    parts = []
    parts.append(f"Summary:\n- Total rows found: {s['total_rows']}")
    if s.get("numeric"):
        for k, v in list(s["numeric"].items())[:5]:
            min_val = int(v['min']) if float(v['min']).is_integer() else round(v['min'], 2)
            max_val = int(v['max']) if float(v['max']).is_integer() else round(v['max'], 2)
            avg_val = int(v['avg']) if float(v['avg']).is_integer() else round(v['avg'], 2)
            parts.append(f"- {k} (numeric): min = {min_val}, max = {max_val}, avg = {avg_val}, zeros = {v['zeros']}")
    if s.get("categorical"):
        for k, v in list(s["categorical"].items())[:5]:
            tops = ", ".join(f"{val}({cnt})" for val, cnt in v["top"])
            parts.append(f"- {k} (categorical): unique={v['unique']}, top={tops}")
    parts.append("\nObservations:\n- Use the Excel link below to download full rows.")
    return "\n".join(parts)
...
def write_styled_excel(rows: List[Dict[str, Any]], filename: str) -> str:
...
# R2 client creation & upload

def make_r2_client():
    access = (R2_ACCESS_KEY_ID or "").strip()
    secret = (R2_SECRET_ACCESS_KEY or "").strip()
    endpoint = os.getenv("R2_S3_ENDPOINT", "").strip()
    if not access or not secret:
        logger.info("R2 credentials not provided; skipping R2 client creation.")
        return None
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"R2 client created but list_objects test failed: {e}")
        return client
    except Exception as e:
        logger.exception(f"Failed to create R2 client: {e}")
        return None
...
def upload_to_r2(local_path: str, filename: str) -> str:
    ...
    if not R2_CLIENT:
        logger.info("No R2 client available; returning local URL.")
        return local_url()

    try:
        with open(local_path, "rb") as fh:
            key = f"{R2_BUCKET}/{filename}"  # IMPORTANT FIX
            R2_CLIENT.put_object(Bucket=R2_BUCKET, Key=key, Body=fh)
    except Exception as e:
        logger.exception(f"R2 upload failed: {e}")
        return local_url()

    # PUBLIC DEV URL FIX — Cloudflare R2 dev URL does NOT include bucket
    if R2_PUBLIC_URL:
    ...
    try:
        ...
        logger.exception(f"R2 presign failed: {e}")
        return local_url()



# Websocket: main flow (integrated intent classifier)
...
async def ws_query(ws: WebSocket):
    ...
    try:
        while True:
            ...
            # 0) Intent classification (Mode B using CLASSIFIER_MODEL)
        
            try:
                intent_prompt = INTENT_CLASSIFIER_PROMPT + f"\n\nUser question: {question}\n\nRespond with exactly one token from: sql_generation, general_answer, both, clarification_needed."
                intent_resp = await asyncio.to_thread(CLASSIFIER_MODEL.invoke, intent_prompt)
                intent = safe_to_text(intent_resp).strip().lower()
                # normalize possible verbose responses
                if "sql_generation" in intent:
                    intent = "sql_generation"
                elif "general_answer" in intent:
                    intent = "general_answer"
                elif "both" in intent:
                    intent = "both"
                elif "clarification_needed" in intent or "clarify" in intent:
                    intent = "clarification_needed"
                else:
                    # fallback heuristic: look for keywords (very conservative)
                    ql = question.lower()
                    if any(k in ql for k in ["sum", "total", "average", "where", "item_no", "inventory", "select", "show me", "list"]):
                        intent = "sql_generation"
                    else:
                        intent = "general_answer"
            except Exception as e:
                logger.exception(f"Intent classification failed: {e}")
                # conservative fallback to general_answer
                intent = "general_answer"

            # If clarification needed, ask user
            if intent == "clarification_needed":
                await ws.send_json({"type": "answer", "message": "Can you clarify what details you want — e.g., a summary, a total of a column, or details for a specific item_no?"})
                continue

            # If purely general, use ANSWER_MODEL for general response and skip SQL
            if intent == "general_answer":
                try:
                    resp = await asyncio.to_thread(ANSWER_MODEL.invoke, question)
                    text = safe_to_text(resp).strip()
                    if not text:
                        text = "I don't have a direct answer. Can you rephrase?"
                    await ws.send_json({"type": "answer", "message": text})
                except Exception as e:
                    logger.exception(f"General answer generation failed: {e}")
                    await ws.send_json({"type": "answer", "message": "Sorry, I couldn't generate an answer right now."})
                continue

            # At this point intent is sql_generation or both
            # 1) Generate SQL only if needed
            ...
            # 3) Ensure item_no included for item queries (fallback full row fetch)
            is_item_query = bool(re.search(r'WHERE\s+"item_no"\s*=', sql or "", flags=re.I))
            if is_item_query:
                if rows and "item_no" not in rows[0].keys():
                    m = re.search(r'WHERE\s+"item_no"\s*=\s*\'([^\']*)\'', sql or "", flags=re.I)
                    if m:
                        item_val = clean_item_literal(m.group(1))
                        full_sql = f'SELECT * FROM {SCHEMA}.{TABLE_NAME} WHERE "item_no" = \'{item_val}\''
                        rows = await exec_sql(full_sql)

            # 4) Build answer: If intent == both, generate a general explanation + data answer
            data_answer = await generate_answer(question, sql or "", rows)

            if intent == "both":
                # generate a short general explanation (non-db) using ANSWER_MODEL but do not access {rows}
                # We'll ask the model to answer the general part only.
                # To be conservative, prepend a clarifier asking for the general portion.
                general_prompt = f"Answer the non-database part of this question briefly:\n\n{question}\n\nIf the question is database-specific, keep the answer general and conceptual. Do NOT access any database."
                try:
                    gen_resp = await asyncio.to_thread(ANSWER_MODEL.invoke, general_prompt)
                    general_text = safe_to_text(gen_resp).strip()
                except Exception:
                    general_text = ""
                # Combine general and data answer
                combined = ""
                if general_text:
                    combined += general_text.strip() + "\n\n"
                combined += data_answer.strip()
                answer = combined
            else:
                answer = data_answer

            # 5) Always create local summary and attach (fallback + convenience)
            summary_text = simple_summary_text(rows)
            chat_text = (answer.strip() if answer else "") + "\n\n" + summary_text

            await ws.send_json({"type": "answer", "message": chat_text})

            # 6) If rows exist → produce Excel, upload to R2 (or fallback) and send URL + preview
            if rows:
                schema_cols = await get_schema()
                available_cols = [c["column_name"] for c in schema_cols]
                requested = extract_requested_fields(question, available_cols)

                # Preferred ordering: item_no (if item query) -> requested fields -> rest
                first_cols = []
                if is_item_query and "item_no" in rows[0].keys():
                    first_cols.append("item_no")
                for rc in requested:
                    if rc in rows[0].keys() and rc not in first_cols:
                        first_cols.append(rc)
                all_cols = list(rows[0].keys())
                ordered_cols = [c for c in first_cols if c in all_cols] + [c for c in all_cols if c not in first_cols]

                ordered_rows = [{c: r.get(c) for c in ordered_cols} for r in rows]

                filename = f"query_{uuid.uuid4().hex[:8]}.xlsx"
                local_path = write_styled_excel(ordered_rows, filename)
                file_url = upload_to_r2(local_path, filename)

                await ws.send_json({"type": "file", "filename": filename, "url": file_url})
                await ws.send_json({"type": "file_preview", "preview_rows": [serialize_row(r) for r in ordered_rows[:8]]})

            # 7) Save memory (store short chat_text summary)
            async with session_lock:
                session_memory[session_id].append({"q": question, "a": chat_text})
